...
function Cientific() {
  ...
  const handleTrigonometricFunction = (func) => {
    try {
      const result = eval(`${func}(${parseFloat(display)})`);
      setDisplay(result.toString());
      setOperation('');
    } catch (error) {
      setDisplay('Error');
    ...
    <div className="flex justify-center items-center h-screen section-kit">
      <div className="bg-gray-200 p-4 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-4 gap-2">
          <button onClick={() => handleTrigonometricFunction('Math.sin')} className="btn">sin</button>
          <button onClick={() => handleTrigonometricFunction('Math.cos')} className="btn">cos</button>
          <button onClick={() => handleTrigonometricFunction('Math.tan')} className="btn">tan</button>
          <button onClick={handleLogarithm} className="btn">log</button>
          <button onClick={() => handleSquareRoot()} className="btn">√</button>
